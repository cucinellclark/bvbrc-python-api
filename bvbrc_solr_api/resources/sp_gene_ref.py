...
class SpGeneRef:
  ...
  async def get_by_id(self, id: str, options: Dict[str, Any] | None = None):
    return await run("sp_gene_ref", qb.eq("id", id), options or {}, self._client, self._ctx["base_url"], self._ctx["headers"])

  async def query_by(self, filters: Dict[str, Any] | None = None, options: Dict[str, Any] | None = None):
    return await run("sp_gene_ref", qb.build_and_from(filters or {}), options or {}, self._client, self._ctx["base_url"], self._ctx["headers"])

  async def get_by_antibiotics(self, antibiotics: str, options: Dict[str, Any] | None = None):
    return await run("sp_gene_ref", qb.eq("antibiotics", antibiotics), options or {}, self._client, self._ctx["base_url"], self._ctx["headers"])

  async def get_by_gene_symbol(self, gene_symbol: str, options: Dict[str, Any] | None = None):
    return await run("sp_gene_ref", qb.eq("gene_symbol", gene_symbol), options or {}, self._client, self._ctx["base_url"], self._ctx["headers"])

  async def get_by_source(self, source: str, options: Dict[str, Any] | None = None):
    return await run("sp_gene_ref", qb.eq("source", source), options or {}, self._client, self._ctx["base_url"], self._ctx["headers"])

  async def get_by_taxon_id(self, taxon_id: int, options: Dict[str, Any] | None = None):
    return await run("sp_gene_ref", qb.eq("taxon_id", taxon_id), options or {}, self._client, self._ctx["base_url"], self._ctx["headers"])

  async def get_by_date_inserted_range(self, start_date: str, end_date: str, options: Dict[str, Any] | None = None):
    return await run("sp_gene_ref", qb.and_(qb.gte("date_inserted", start_date), qb.lte("date_inserted", end_date)), options or {}, self._client, self._ctx["base_url"], self._ctx["headers"])

  async def get_by_date_modified_range(self, start_date: str, end_date: str, options: Dict[str, Any] | None = None):
    return await run("sp_gene_ref", qb.and_(qb.gte("date_modified", start_date), qb.lte("date_modified", end_date)), options or {}, self._client, self._ctx["base_url"], self._ctx["headers"])

  async def search_by_keyword(self, keyword: str, options: Dict[str, Any] | None = None):
    return await run("sp_gene_ref", f"keyword({quote(keyword)})", options or {}, self._client, self._ctx["base_url"], self._ctx["headers"])

  async def get_all(self, options: Dict[str, Any] | None = None):
    return await run("sp_gene_ref", "", options or {}, self._client, self._ctx["base_url"], self._ctx["headers"])
