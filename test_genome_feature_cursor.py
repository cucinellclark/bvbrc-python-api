...
def parse_args():
  parser = argparse.ArgumentParser(description="Stream genome features via Solr cursor")
  ...
  parser.add_argument("--sort", dest="sort", default=None, help="Sort expression (e.g., 'feature_id asc'). Defaults to unique key asc")
  parser.add_argument("--unique-key", dest="unique_key", default="feature_id", help="Unique key field for stable sort (default: feature_id)")
  ...
  parser.add_argument("--max-docs", dest="max_docs", type=int, default=-1, help="Max docs to print before stopping (default: 20; use -1 for no limit)")
  parser.add_argument("--pretty", dest="pretty", type=int, default=2, help="Indentation for JSON pretty print (default: 2)")
  parser.add_argument("--genome-id", dest="genome_id", default="208964.12", help="Genome ID to filter by (default: 208964.12)")
...
def main():
  ...
  fq_list = list(args.fq) if args.fq else []
  
  # Add genome_id filter to get features from specific genome
  fq_list.append(f"genome_id:{args.genome_id}")
  ...
  try:
    ...
    print(f"\nPrinted {printed} document(s) from genome_id {args.genome_id}.", file=sys.stderr)
